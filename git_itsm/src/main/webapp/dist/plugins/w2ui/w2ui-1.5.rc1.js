        ...
        return $(this).each(function (index, el) {
            ...
            setTimeout(function () {
                ...
                if (options.hideOnKeyPress) {
                    //$(el).on('keypress.w2tag', hideTag); //jini orginal
                	$(el).on('keyup.w2tag', hideTag);
                }
                    ...
                    setTimeout(function () {
                        ...
                        if (edit.type == 'list') {
                            ...
                            if (typeof val != 'object' && val != '') {
                            	//jini
                            	if(edit.hasOwnProperty("customDisable")){
                            		if(edit.customDisable){
                            			tmp.val(val).css({ opacity: 0 }).prev().css({ opacity: 1 });
                            		}
                            	}else{
                            		tmp.val(val).css({ opacity: 1 }).prev().css({ opacity: 1 });
                            	}
...
//                    break;
        ...
        getCellValue: function (ind, col_ind, summary) {
            ...
            if(this.hasOwnProperty("blankSymbol")){
            	if(data === "") data = this.blankSymbol; //jini
            }
    ...
    w2popup = {
        ...
        open: function (options) {
            ...
            function mvStop(evnt) {
                ...
                /*console.log("tmp.div_y    : "+ tmp.div_y);
                console.log("tmp.pos_y    : "+ tmp.pos_y);*/
                ...
                /*jini*/
                var currentWidth =  $('#w2ui-popup').width() +2; //outer glow area 2px
                var currentHeight = $('#w2ui-popup').height() + 2;
                var leftRightValue = tmp.pos_x + tmp.div_x;
                var topBottomValue = tmp.pos_y + tmp.div_y;
                
                if(leftRightValue < 0 )leftRightValue = 0;
                if(topBottomValue < 0 ) topBottomValue = 0;
                
                if( leftRightValue+currentWidth > window.innerWidth) leftRightValue = window.innerWidth - currentWidth;
                if(topBottomValue+currentHeight > window.innerHeight) topBottomValue = window.innerHeight - currentHeight;
                
                $('#w2ui-popup').css({
                	'left': leftRightValue + 'px',
                	'top' : topBottomValue + 'px'
                }).css(w2utils.cssPrefix({
                	'transition': 'none',
                	'transform' : 'translate3d(0px, 0px, 0px)'
                }));
        ...
        refresh: function (id) {
            ...
            function getNodeHTML(nd) {
                ...
                if (nd.group) {
                    ...
                    if(nd.draggable){
                    	html =  '<div draggable="true" class="w2ui-node w2ui-level-'+ level +' '+ (nd.selected ? 'w2ui-selected' : '') +' '+ (nd.disabled ? 'w2ui-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
                        ...
                        '<td class="w2ui-node-data" nowrap="nowrap">'+
                                ...
                                '<div class="w2ui-node-caption" style="'+nd.nodeStyle+'">'+ text +'</div>'+
                        '</td>'+
                        '</tr></tbody></table>'+
                        '</div>'+
                        '<div class="w2ui-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    ...
                    if (obj.flat) {
                	  if(nd.draggable){
                          ...
                          '    oncontextmenu="w2ui[\''+ obj.name +'\'].contextMenu(\''+ nd.id +'\', event);"'+
                          '    onClick="w2ui[\''+ obj.name +'\'].click(\''+ nd.id +'\', event); ">'+
                          '<div class="w2ui-node-data w2ui-node-flat '+ nd.text +'" yesid="'+nd.id+'"'+'yestext="'+nd.text+'" yesdrag="'+nd.draggable+'">'+ tmp +'</div>'+
                          '</div>'+
                          '<div class="w2ui-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';  
                	  }else{
                		  html =  '<div class="w2ui-node w2ui-level-'+ level +' '+ (nd.selected ? 'w2ui-selected' : '') +' '+ (nd.disabled ? 'w2ui-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
      ...
      //focus Out 일때 jini
        blur: function (event) {
            ...
            if (['date', 'time', 'datetime'].indexOf(obj.type) != -1) {
            	...
            	var formatter = $(obj.el).attr("placeholder");
                ...
                (function refreshCalendar(month, year) {
                    ...
                    $('#w2ui-overlay .w2ui-date')
                        .on('mousedown', function () {
                            ...
                            $(this).css({ 'background-color': '#3c567d', 'border-color': '#aaa' });  //jini B6D5FB
                        })
                        .on('mouseup', function () {
                            setTimeout(function () {
                                if ($("#w2ui-overlay").length > 0) $('#w2ui-overlay').removeData('keepOpen')[0].hide();
